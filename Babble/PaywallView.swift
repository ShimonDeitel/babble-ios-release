...
struct PaywallView: View {
    ...
    private let benefits: [(String, String, String)] = [
        ("square.stack.3d.up.fill", "Unlimited collections", "Keep separate shortlists - Boys, Girls, Unisex, or a top-picks list."),
        ("line.3.horizontal.decrease.circle.fill", "Filters", "Narrow the deck by origin and by short, medium or long names."),
        ("person.2.fill", "Partner share", "Send your family shortlist card to a partner in one tap."),
        ("heart.fill", "Support Babble", "A one-time purchase that keeps the app ad-free and private.")
    ]
            ...
            ScrollView {
                VStack(spacing: 22) {
                    VStack(spacing: 8) {
                        ...
                        Text("Babble Pro").font(.largeTitle.weight(.heavy))
                    ...
                    VStack(spacing: 12) {
                        Button { Task { await buy() } } label: {
                            HStack {
                                ...
                                Text(working ? "Unlocking…" : "Unlock Babble Pro · \(store.displayPrice)")
                                    .font(.headline)
                            }
                        ...
                        Text("Babble never tracks you. Your shortlist stays on your device.")
