...
/// StoreKit 2 — one-time non-consumable unlock (`babble_pro_unlock`).
...
final class Store: ObservableObject {
    static let productID = "babble_pro_unlock"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["BABBLE_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            // BABBLE_NO_SK skips the product fetch so headless screenshot runs (no StoreKit config /
            // no App Store account) don't trigger the system "Sign in to Apple Account" prompt.
            if ProcessInfo.processInfo.environment["BABBLE_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
    ...
    #if DEBUG
    /// Test-only: drive the Pro state deterministically. A live SKTestSession purchase hangs in
    /// headless CI, so unit tests flip Pro through this hook to exercise the gated paths.
